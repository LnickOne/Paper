...
"""使用字节对编码（BPE）学习文本中词汇的可变长度编码。
与原始 BPE 不同，它不会压缩纯文本，但可以用于将文本的词汇量减少到可配置的符号数量，同时只略微增加标记数量。
...
def update_pair_statistics(pair, changed, stats, indices):
    ...
    如果我们合并一对符号，只有与该对符号的出现重叠的对会受到影响，需要更新。
    """
    stats[pair] = 0
    ...
    with codecs.open(outfile_name, "w", encoding="utf-8") as outfile:
        ...
        for i in range(num_symbols):
            ...
            if stats[most_frequent] < min_frequency:
                sys.stderr.write(
                    f"no pair has frequency >= {min_frequency}. Stopping\n"
                )
                break
