...
def encode_file(bpe, in_file, out_file):
    sys.stderr.write(
        f"Read raw content from {in_file} and \n"
...
def process_en_zh_txt(input_path, out_en, out_zh):
    """将en-zh-v1.txt分割为英文和中文两个文件"""
    with open(input_path, "r", encoding="utf-8") as fin, open(
        out_en, "w", encoding="utf-8"
    ) as f_en, open(out_zh, "w", encoding="utf-8") as f_zh:
        for line in fin:
            line = line.strip()
            if not line:
                continue
            ...
            if len(parts) == 2:
                f_en.write(parts[0].strip() + "\n")
                f_zh.write(parts[1].strip() + "\n")
            else:
                # 否则尝试用第一个中文字符分割
                match = re.search(r"[\u4e00-\u9fff]", line)
                if match:
                    idx = match.start()
                    f_en.write(line[:idx].strip() + "\n")
                    f_zh.write(line[idx:].strip() + "\n")
                else:
                    continue
...
def main():
    ...
    parser.add_argument(
        "--symbols", "-s", type=int, default=32000, help="Vocabulary size"
    )
    parser.add_argument(
        "--min-frequency",
        type=int,
    ...
    parser.add_argument("--en_zh_txt", type=str, default=None, help="en-zh-v1.txt路径")
    parser.add_argument(
        "--en_zh_out_prefix", type=str, default="train", help="输出文件前缀"
    )
    ...
    if opt.en_zh_txt:
        en_path = os.path.join(opt.data_dir, opt.en_zh_out_prefix + ".en")
        zh_path = os.path.join(opt.data_dir, opt.en_zh_out_prefix + ".zh")
        process_en_zh_txt(opt.en_zh_txt, en_path, zh_path)
        print(f"[Info] 已生成: {en_path}, {zh_path}")
        return
