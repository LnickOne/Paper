...
"""使用 learn_bpe.py 学习的操作对新文本进行编码。
文本不会变小，但只会使用固定词汇表，稀有词会被编码为可变长度的子词单元序列。
...
class BPE(object):
    ...
    def __init__(self, codes, merges=-1, separator="@@", vocab=None, glossaries=None):
        ...
        self.glossaries_regex = (
            re.compile("^({})$".format("|".join(glossaries))) if glossaries else None
        )
        self.cache = {}
...
def encode(
    ...
    while len(word) > 1:
        ...
        pairs = [
            ...
            if (not dropout or random.random() > dropout) and pair in bpe_codes
        ]
