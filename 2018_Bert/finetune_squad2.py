"""
BERT 微调 SQuAD v2.0 阅读理解（含无答案问题）
复现论文: BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding (2018)
对应论文 Table 2 - SQuAD 2.0 任务

与 v1.1 的核心区别：
- 部分问题无答案（is_impossible=True），需额外预测"是否有答案"
- 无答案时答案位置标为 [CLS]（位置 0）
- 评估时需比较"有答案得分"与"无答案得分"，选分数更高的
"""
...
def postprocess_predictions(examples, features, raw_predictions,
    ...
    for example_index, example in enumerate(examples):
        ...
        # 收集所有 window 的最小无答案得分（start[CLS] + end[CLS]）
        min_null_score = None
        valid_answers = []
        ...
        for fi in feature_indices:
            ...
            # 无答案得分 = start_logit[0] + end_logit[0]（[CLS] 位置）
        ...
        if valid_answers:
            ...
            # SQuAD 2.0：若最佳答案得分 - 无答案得分 < 阈值，则预测无答案
            if best["score"] - min_null_score < null_score_diff_threshold:
...
def compute_squad_metrics(predictions, references):
    ...
    for pred_id, pred_text in predictions.items():
        ref = next(r for r in references if r["id"] == pred_id)
        ref_answers = ref["answers"]["text"]
        pred_norm = normalize_answer(pred_text)

        # 无答案情况
        if len(ref_answers) == 0:
            em_scores.append(int(pred_norm == ""))
            f1_scores.append(1.0 if pred_norm == "" else 0.0)
            continue
...
print(f"论文目标 (BERT-Base SQuAD 2.0): EM 75.1%, F1 80.0%\n")
